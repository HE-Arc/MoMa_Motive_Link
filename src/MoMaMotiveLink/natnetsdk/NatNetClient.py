...
import copy
import time
from MoMaMotiveLink.natnetsdk import DataDescriptions, MoCapData
...
class NatNetClient:
    ...
    def __create_command_socket(self):
        ...
        if self.use_multicast:
            ...
            try:
                ...
                # result.bind((self.local_ip_address, self.command_port))
                result.bind((self.local_ip_address, self.unicast_listen_port))
            except socket.error as e:
                print(f"Socket error: {e}")
        return result
    ...
    def __create_data_socket(self):
        ...
        if self.use_multicast:
            ...
            # Unicast case
            ## Note : Ne sert à rien en mode Unicast. Tout passe par le command_socket
            ...
            try:
                # result.bind((self.local_ip_address, 0))
                # result.bind((self.local_ip_address, self.data_port))
                result.bind((self.local_ip_address, 1511))
            except socket.error as e:
    ...
    def __unpack_rigid_body_descript_4_2_to_current(self, data):
        ...
        offset += 16
        rb_desc.set_rot(quat[0], quat[1], quat[2], quat[3])
        trace_dd(
    ...
    def __command_thread_function(self, in_socket : socket.socket, stop, gprint_level, thread_option):  # type: ignore  # noqa E501
        message_id_dict = {}
        ...
        while not stop():
            ...
            try:
                ...
                print("Données reçues sur le socket des commandes")
                ...
                print(f"Timeout duration is {in_socket.timeout}")
                traceback.print_exc()
                # return 4
                ...
                print(
                    f"ERROR: command socket access exception of type {e.__class__.__name__} occurred: {e}"
                )  # type: ignore  # noqa E501
                traceback.print_exc()
                return 5
            ...
            if len(buffer_list[buffer_list_in_use_index]) > 0:
                ...
                try:
                    ...
                    print(
                        f"Exception évitée dans process_message (Trame corrompue ?) : {e}"
                    )
            ...
            if not self.use_multicast:
                if not stop():
                    ...
                    # N'envoyer le ping que toutes les 2 secondes
                    current_time = time.time()
                    if current_time - last_keep_alive > 2.0:
                        print(
                            f"[{current_time}] PING envoyé vers {self.server_ip_address}:{self.command_port}"
                        )
                        try:
                            ...
                            print(
                                f"Erreur réseau ignorée lors de l'envoi du Keep-Alive : {e}"
                            )
    ...
    def __data_thread_function(self, in_socket, stop, gprint_level):
        ...
        while not stop():
            try:
                ...
                # print("Données reçues sur le socket de données")
                ...
                print("ERROR: data socket access exception occurred: %s" % str(e))  # type: ignore  # noqa E501
                # return 5
            if len(data) > 0:
                # peek ahead at message_id
    ...
    def __process_message(self, data: bytes, print_level=0):
        ...
        if message_id == self.NAT_FRAMEOFDATA:
            ...
            # Custom Event to handle the data descriptions when they are received. T
            # his allows users to have a callback for when the data descriptions are updated
            # The data descriptions will be passed as an argument to the callback function.
            if self.model_description_listener is not None:
                self.model_description_listener(data_descs)
    ...
    def send_request(self, in_socket, command, command_str, address):
        ...
        if command == self.NAT_CONNECT:
            ...
            data += b"\0"
        elif packet_size > 0:  # On n'ajoute la chaîne et le \0 QUE s'il y a un payload
